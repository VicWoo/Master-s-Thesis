...
The automation of semantic segmentation will not only help cut down on the time significantly but help reduce intra-observer and inter-observer variability in the segmentation results. 
The main challenge to automate semantic segmentation is the lack of sufficient data with the ground truth. Manual segmentation is often treated as the ground truth, but intra-observer and inter-observer variability as render the segmented regions to be inconsistent. 
It is also very time consuming to preform manual segmentation on a large number of data sets.
Recent advances in semantic segmentation of natural and medical images like more efficient models and better data augmentation techniques allows for the mitigation of these challenges.
\par
...
This thesis will investigate multiple deep convolutional neural networks that have had success in semantic segmentation and apply them to the rat brain MR images.
We will first consider single region segmentation using convolutional neural networks and then the networks are applied to semantically segment the MR image into 14 different brain regions.
\par
...
The results also show that automatic segmentation by the deep convolutional neural networks was comparable to the manual segmentation of the rat brain MR images by a trained human observer. 
